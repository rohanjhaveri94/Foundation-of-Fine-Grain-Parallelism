...
/* We do not support C11 <threads.h>.  */
  ...
  int **d_a, **d_b, **d_c; 
	...
	matrixMult<<<dimGrid, dimBlock>>>(dev_a, dev_b, dev_c, _PB_NI,_PB_NJ,_PB_NK,alpha); 
...
__global__ void matrixMult (int **A, int **B, int **C, int _PB_NI, int _PB_NJ, int _PB_NK, DATA_TYPE alpha) 
{ 
	int c2, c3, c4, c5 = 0;
	
	int c1col = threadIdx.x + blockDim.x * blockIdx.x; 
	int c0row = threadIdx.y + blockDim.y * blockIdx.y; 
